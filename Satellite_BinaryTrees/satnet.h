...
class SatNet{
    ...
    void postOrder(Sat* satellite);
    void insertRecur(const Sat& satellite, Sat*& sat);
    void updateHeight(Sat* sat);
    void checkImbalance(Sat*& sat);
    void rightRotation(Sat*& sat);
    void leftRotation(Sat*& sat);
    void inOrder(Sat* satellite) const;
    Sat* removeRecur(Sat*& aSat, int& id);
    Sat* findMin(Sat* aSat);
    bool setStateRecur(int id, STATE state, Sat* satellite);
    void removeDeorbitedRecur(Sat* satellite);
    int countSatellitesRecur(INCLIN degree, Sat* satellite) const;
    bool findSatelliteRecur(Sat* aSat, int id) const;
    void preOrder(Sat*& aSat, Sat* rhs);
    int countSatellitesDeorbited(Sat* satellite);
    // ***************************************************
